...
def add_Expini():
    TheseAddons   =  [Exp]#Jules: only run for Exp!!
...
def ParseExpData(FoundAddon):
    ...
    for channel in ChannelsResult:
        ...
        stream  = channel['file']
        ChannelURL  = GetExpStuff(FoundAddon, ParsedChannels)
        channel = RemoveExpChanCrap(FoundAddon, ChannelURL)
    ...
    Clean_Names_Exp(Addoninipath,tmp_File)
#
def GrabSettingsFromExp(FoundAddon):
    ...
    endOfString    =  GetExpVariables(FoundAddon)
...
#    
def GetExpStuff(FoundAddon, RemoveURLGarbage):
    RemoveURLGarbage = RemoveURLGarbage.replace('COLOR+', '').replace(' [/B]', '').replace('[COLOR white]', '').replace('[/COLOR]', '').replace('[COLOR firebrick]', '').replace('COLOR+steelblue', '')      
    return RemoveURLGarbage
#    
def RemoveExpChanCrap(FoundAddon, FoundChannels):
    channel = FoundChannels.rsplit('[/B]', 1)[0].split('[B]', 1)[-1]        
    return channel
#       
def GetExpVariables(FoundAddon):
    Addon    =  xbmcaddon.Addon(FoundAddon) 
...
# PVR End #
def Clean_Names_Exp(Clean_Name,tmpFile):
