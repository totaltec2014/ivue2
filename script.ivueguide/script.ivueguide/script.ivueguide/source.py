...
class XMLTVSource(Source):
	...
	LOGO_URL = addon.getSetting('logos')	
        ...
        if self.addonsType == XMLTVSource.INI_TYPE_IVUE:
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            xbmc.log('[script.ivueguide] Date to normalize: ' + dateString, xbmc.LOGDEBUG)
            try:
            ...
            t = t + td_local
            xbmc.log('[script.ivueguide] Import Time adjusted from: ' + str(t_tmp) + ' to: ' + str(t), xbmc.LOGDEBUG)
            return t
